...
const App=(()=>{
  ...
  async function onSignedIn(user,token){
    ...
    try{
      // Guardar los datos en la carpeta COMPARTIDA para que Santiago y Elizabeth
      // vean los mismos clientes, tareas y notas. Si no hay carpeta compartida
      // configurada, se cae a una carpeta personal "Ceneca Workspace".
      if(CONFIG.SHARED_FOLDER_ID){
        _root=CONFIG.SHARED_FOLDER_ID;
      }else{
        _root=await Drive.ensureFolder(CONFIG.DRIVE_ROOT_FOLDER);
      }
      await Data.init(_root);
    }
