...
const Auth = (() => {
  ...
  function init(){
    return new Promise(resolve=>{
      ...
      s.onload=()=>{
        google.accounts.id.initialize({client_id:CONFIG.CLIENT_ID,callback:_cred,auto_select:false});
        _tokenClient=google.accounts.oauth2.initTokenClient({
          client_id:CONFIG.CLIENT_ID,scope:CONFIG.SCOPES,
          callback:r=>{
            if(r.access_token){
              _token=r.access_token;localStorage.setItem('ws_token',_token);
              const u=_user||JSON.parse(localStorage.getItem('ws_user')||'null');
              if(u&&u.email)App.onSignedIn(u,_token);
            }
          }
        });
        const st=localStorage.getItem('ws_token'),su=localStorage.getItem('ws_user');
        if(st&&su){_token=st;_user=JSON.parse(su);}
        resolve();
      };
      s.onerror=()=>resolve();
  ...
  function signIn(){
    ...
    if(su&&st&&st!=='demo'){_user=JSON.parse(su);_token=st;App.onSignedIn(_user,_token);return;}
