...
const Drive = (() => {
  ...
  // Asegura que exista una carpeta; la crea si no existe
  ...
  //   <raíz>/
  //     _data/           ← JSONs del sistema (tareas, clientes, notas, actividad)
  //     Clientes/
  //       <NombreCliente>/  ← archivos de ese cliente
  let _dataFolderId=null;
  let _clientesFolderId=null;

  async function getDataFolder(root){
    if(_dataFolderId)return _dataFolderId;
    _dataFolderId=await ensureFolder('_data',root);
    return _dataFolderId;
  }
  async function getClientesFolder(root){
    if(_clientesFolderId)return _clientesFolderId;
    _clientesFolderId=await ensureFolder('Clientes',root);
    return _clientesFolderId;
  }

  // Crea (o reutiliza) la carpeta de un cliente específico dentro de Clientes/
  async function ensureClientFolder(nombre,root){
    if(Auth.isDemo())return 'demo-cliente-'+nombre;
    const parentId=await getClientesFolder(root);
    return ensureFolder(nombre,parentId);
  }
  ...
  // Borra un archivo en Drive. Si se pasa clienteId, también elimina su registro del JSON de clientes.
  async function deleteFile(id){
    if(Auth.isDemo())return;
    await fetch(`${B}/files/${id}`,{method:'DELETE',headers:_h()});
  }

  // Borra una carpeta entera (usada al eliminar cliente)
  async function deleteFolder(id){
    if(Auth.isDemo())return;
    await fetch(`${B}/files/${id}`,{method:'DELETE',headers:_h()});
  }

  // Guarda un JSON en la subcarpeta _data/
  async function saveData(fn,data,root){
    if(Auth.isDemo()){localStorage.setItem('ws_d_'+fn,JSON.stringify(data));return;}
    const dataFolder=await getDataFolder(root);
    const q=`name='${fn}' and '${dataFolder}' in parents and trashed=false`;
    const ex=await _r(`${B}/files?q=${encodeURIComponent(q)}&fields=files(id)`);
    if(ex.files.length){
      await fetch(`${U}/files/${ex.files[0].id}?uploadType=media`,{method:'PATCH',headers:_h(),body:JSON.stringify(data)});
    }else{
      const form=new FormData();
      form.append('metadata',new Blob([JSON.stringify({name:fn,parents:[dataFolder]})],{type:'application/json'}));
      form.append('file',new Blob([JSON.stringify(data)],{type:'application/json'}));
      await fetch(`${U}/files?uploadType=multipart`,{method:'POST',headers:{Authorization:`Bearer ${Auth.getToken()}`},body:form});
    }
  }

  // Lee un JSON de la subcarpeta _data/
  ...
  // Resetea caché de IDs de carpetas (necesario si se cambia de cuenta)
  function resetFolderCache(){_dataFolderId=null;_clientesFolderId=null;}
  ...
  return{ensureFolder,ensureClientFolder,getClientesFolder,listFiles,uploadFile,downloadUrl,webViewLink,getVersions,deleteFile,deleteFolder,saveData,loadData,resetFolderCache};
})();
...
const Data = (() => {
  ...
  async function agregarCliente(c){const n={...c,id:_nid(_db.clientes),creadoEn:new Date().toISOString()};_db.clientes.push(n);await _save('clientes');// Crear carpeta del cliente en Drive y guardar su ID
if(_root){try{const fid=await Drive.ensureClientFolder(n.nombre,_root);n.driveFolderId=fid;await _save('clientes');}catch(e){console.warn('No se pudo crear carpeta del cliente',e);}}agregarActividad('cliente',`<strong>Cliente agregado:</strong> ${n.nombre}`,'👤','#faeeda');return n;}
  ...
  async function eliminarCliente(id){const c=_db.clientes.find(c=>c.id===id);_db.clientes=_db.clientes.filter(c=>c.id!==id);// Borrar archivos de Drive del cliente
if(c?.driveFolderId){try{await Drive.deleteFolder(c.driveFolderId);}catch(e){console.warn('No se pudo borrar carpeta del cliente en Drive',e);}}await _save('clientes');if(c)agregarActividad('cliente',`Cliente <strong>eliminado:</strong> ${c.nombre}`,'🗑','#fcebeb');}
  // notas
...
const Tareas={_filtro:'todas',render(el){
  ...
  el.innerHTML=`<div class="view active">
    ...
    <div style="display:flex;gap:6px;margin-bottom:20px;flex-wrap:wrap">${['todas','santiago','elizabeth','vencidas','hoy'].map(f=>`<button class="btn btns${this._filtro===f?' btnp':''}" onclick="Tareas.setFiltro('${f}')">${({todas:'Todas',santiago:'Santiago',elizabeth:'Elizabeth',vencidas:'Vencidas',hoy:'Para hoy'})[f]}</button>`).join('')}</div>
...
const Calendario={
  _a:new Date().getFullYear(),_m:new Date().getMonth(),_eventos:[],_eventosCeneca:[],_errorCeneca:null,
  async render(el){
    el.innerHTML=`<div class="view active">
      <div class="ph"><div><div class="pt">Calendario</div><div class="psub">Tareas y eventos del workspace</div></div>
      <button class="btn btnp" onclick="Tareas.nueva()">+ Nueva tarea</button></div>
      <div style="display:flex;gap:8px;margin-bottom:16px">
        <div style="display:flex;align-items:center;gap:6px;font-size:12px;color:var(--text2)"><div style="width:12px;height:12px;border-radius:3px;background:var(--accent)"></div>Tareas abiertas</div>
        <div style="display:flex;align-items:center;gap:6px;font-size:12px;color:var(--text2)"><div style="width:12px;height:12px;border-radius:3px;background:var(--red)"></div>Vencidas</div>
      </div>
      <div class="card">
  ...
  _grid(){
    ...
    for(let d=1;d<=days;d++){
      ...
      const ov=dt.some(t=>ds<hoy);
      h+=`<div class="cday${ds===hoy?' today':''}"><div class="dnum">${d}</div>
        ${dt.slice(0,2).map(t=>{const venc=t.fecha<hoy;return`<div class="cev ${venc?'':'cevg'}" style="${venc?'background:var(--red-l);color:var(--red);':''}" title="${U.esc(t.titulo)}">${t.titulo}</div>`;}).join('')}
        ${dt.length>2?`<div class="cev cevg">+${dt.length-2} más</div>`:''}
      </div>`;
    }
  ...
  _renderListas(){
    ...
    if(elC){
      const tareas=Data.getTareas().filter(t=>!t.done&&t.fecha).sort((a,b)=>a.fecha.localeCompare(b.fecha)).slice(0,8);
      if(!tareas.length){elC.innerHTML='<div style="color:var(--text3);font-size:13px">Sin tareas próximas</div>';}
      else{elC.innerHTML=tareas.map(t=>{const cl=Data.getCliente(t.clienteId);const venc=t.fecha<hoy;const al={santiago:'Santiago',elizabeth:'Elizabeth',ambos:'Ambos'}[t.asignado]||t.asignado;return`<div style="display:flex;gap:10px;padding:9px 0;border-bottom:1px solid var(--border);align-items:center"><div style="width:8px;height:8px;border-radius:50%;background:${venc?'var(--red)':'var(--accent)'};flex-shrink:0"></div><div style="flex:1"><div style="font-size:13px">${t.titulo}</div><div style="font-size:11px;color:var(--text3);margin-top:2px">${venc?'<span style="color:var(--red)">Vencida</span> · ':''}${U.fecha(t.fecha)}${cl?' · '+cl.emoji+' '+cl.nombre:''} · ${al}</div></div></div>`;}).join('');}
    }
  },
...
// ---------- VISTA: ARCHIVOS ----------
// Tipos de filtro: cada entrada define qué mimeTypes acepta (null = todos)
const FILTROS_TIPO=[
  {id:'todos',   label:'Todos',          icon:'📁', mime:null},
  {id:'pdf',     label:'PDFs',           icon:'📄', mime:['application/pdf']},
  {id:'doc',     label:'Documentos',     icon:'📝', mime:['application/vnd.openxmlformats-officedocument.wordprocessingml.document','application/msword','application/vnd.google-apps.document']},
  {id:'sheet',   label:'Hojas de cálculo',icon:'📊',mime:['application/vnd.openxmlformats-officedocument.spreadsheetml.sheet','application/vnd.ms-excel','application/vnd.google-apps.spreadsheet']},
  {id:'img',     label:'Imágenes',       icon:'🖼', mime:null, startsWith:'image/'},
  {id:'video',   label:'Videos',         icon:'🎬', mime:null, startsWith:'video/'},
  {id:'ppt',     label:'Presentaciones', icon:'📋', mime:['application/vnd.openxmlformats-officedocument.presentationml.presentation','application/vnd.ms-powerpoint','application/vnd.google-apps.presentation']},
];

const Archivos={
  _cur:null,_crumb:[],_filtro:'todos',_filesCache:[],
  render(el){
    el.innerHTML=`<div class="view active">
      <div class="ph">
        <div><div class="pt">Archivos</div><div class="psub">Carpeta compartida del equipo</div></div>
        <div style="display:flex;gap:8px">
          <button class="btn" onclick="Archivos.nuevaCarpeta()">+ Nueva carpeta</button>
          <button class="btn btnp" onclick="document.getElementById('fup').click()">↑ Subir archivo</button>
        </div>
      </div>
      <input type="file" id="fup" style="display:none" multiple onchange="Archivos.subir(this.files)" />
      <div class="bc" id="bc"></div>
      <div id="ftabs" style="display:flex;gap:6px;margin-bottom:16px;flex-wrap:wrap"></div>
      <div class="uz" id="uz" ondragover="event.preventDefault();this.classList.add('drag')" ondragleave="this.classList.remove('drag')" ondrop="event.preventDefault();this.classList.remove('drag');Archivos.subir(event.dataTransfer.files)" onclick="document.getElementById('fup').click()">
        <div style="font-size:28px;margin-bottom:8px">📂</div>
        <div style="font-size:14px;font-weight:500">Arrastra archivos aquí o haz clic para subir</div>
        <div style="font-size:12px;color:var(--text3);margin-top:4px">PDFs, documentos, imágenes — cualquier tipo de archivo</div>
      </div>
      <div id="fc"></div>
    </div>`;
    this._loadRoot();
  ...
  async _loadRoot(){
    const r=App.rootFolderId();
    if(!r){this._vacio('Conecta Google Drive para ver archivos');return;}
    try{
      const clientesId=await Drive.getClientesFolder(r);
      this._cur=clientesId;
      this._crumb=[{id:r,name:'Archivos'},{id:clientesId,name:'Clientes'}];
      this._bc();
      await this._cargar(clientesId);
    }catch(e){
      this._cur=r;this._crumb=[{id:r,name:'Archivos'}];this._bc();await this._cargar(r);
    }
  },
  async _cargar(id){
    document.getElementById('fc').innerHTML='<div style="padding:20px;text-align:center;color:var(--text3)">Cargando...</div>';
    let files=[];
    try{files=await Drive.listFiles(id);}catch(e){this._vacio('No se pudieron cargar los archivos');return;}
    this._filesCache=files;
    this._filtro='todos';
    this._renderTabs();
    this._archivos(files);
  },
  // Devuelve true si el archivo coincide con el filtro activo
  _matchFiltro(f){
    if(this._filtro==='todos')return true;
    const ft=FILTROS_TIPO.find(t=>t.id===this._filtro);
    if(!ft)return true;
    if(ft.startsWith)return f.mimeType&&f.mimeType.startsWith(ft.startsWith);
    return ft.mime&&ft.mime.includes(f.mimeType);
  },
  _renderTabs(){
    const el=document.getElementById('ftabs');
    if(!el)return;
    // Solo mostrar tabs si estamos dentro de una carpeta de cliente (no en la raíz/Clientes)
    const docs=this._filesCache.filter(f=>f.mimeType!=='application/vnd.google-apps.folder');
    if(!docs.length){el.innerHTML='';return;}
    // Contar archivos por tipo para mostrar solo tabs relevantes
    const conteos={todos:docs.length};
    FILTROS_TIPO.slice(1).forEach(ft=>{
      const n=docs.filter(f=>ft.startsWith?f.mimeType?.startsWith(ft.startsWith):ft.mime?.includes(f.mimeType)).length;
      if(n>0)conteos[ft.id]=n;
    });
    el.innerHTML=Object.entries(conteos).map(([id,n])=>{
      const ft=FILTROS_TIPO.find(t=>t.id===id);
      const activo=this._filtro===id;
      return`<button class="btn btns${activo?' btnp':''}" onclick="Archivos.setFiltro('${id}')">${ft.icon} ${ft.label} <span style="opacity:.7">(${n})</span></button>`;
    }).join('');
  },
  setFiltro(id){
    this._filtro=id;
    this._renderTabs();
    this._archivos(this._filesCache);
  },
  _archivos(files){
    const c=document.getElementById('fc');
    if(!files.length){c.innerHTML='<div class="empty"><div class="eic">📁</div><div class="etitle">Carpeta vacía</div><div class="esub">Sube un archivo o crea una subcarpeta</div></div>';return;}
    const carpetas=files.filter(f=>f.mimeType==='application/vnd.google-apps.folder');
    const docs=files.filter(f=>f.mimeType!=='application/vnd.google-apps.folder'&&this._matchFiltro(f));
    if(!carpetas.length&&!docs.length){
      c.innerHTML='<div class="empty"><div class="eic">🔍</div><div class="etitle">Sin archivos de este tipo</div><div class="esub">Prueba otro filtro</div></div>';return;
    }
    c.innerHTML=`${carpetas.length&&this._filtro==='todos'?`<div class="slabel">Carpetas</div><div style="margin-bottom:16px">${carpetas.map(f=>this._fila(f,true)).join('')}</div>`:''}${docs.length?`<div class="slabel">Archivos (${docs.length})</div><div>${docs.map(f=>this._fila(f,false)).join('')}</div>`:''}`;
  },
  _fila(f,isC){
    const click=isC?`Archivos.abrirCarpeta('${f.id}','${U.esc(f.name)}')`:`Archivos.previsualizarArchivo('${f.id}','${U.esc(f.name)}','${f.mimeType}')`;
    return`<div class="frow" onclick="${click}"><div class="fic">${U.iconoArchivo(f.mimeType)}</div><div class="fnm">${f.name}</div><div class="fmt">${isC?'':U.peso(f.size)}</div><div class="fmt" style="margin-left:12px">${isC?'':U.fecha(f.modifiedTime)}</div>${!isC?`<div class="fract">${f.version&&parseInt(f.version)>1?`<button class="ibtn" onclick="event.stopPropagation();Archivos.versiones('${f.id}','${U.esc(f.name)}')" title="Historial">v${f.version}</button>`:''}<button class="ibtn" onclick="event.stopPropagation();Archivos.eliminar('${f.id}','${U.esc(f.name)}')">×</button></div>`:''}</div>`;
  },
  async abrirCarpeta(id,name){this._cur=id;this._crumb.push({id,name});this._bc();await this._cargar(id);},
  async previsualizarArchivo(id,name,mime){
    const pdf=mime==='application/pdf',img=mime&&mime.startsWith('image/');
    if(pdf||img){
      if(Auth.isDemo()){App.modal(name,`<div class="empty"><div class="eic">${pdf?'📄':'🖼'}</div><div class="esub">Vista previa disponible al conectar Drive</div></div>`,null,true);return;}
      const url=Drive.downloadUrl(id);
      App.modal(name,pdf?`<iframe src="${url}" style="width:100%;height:500px;border:none;border-radius:6px"></iframe>`:`<img src="${url}" style="max-width:100%;border-radius:6px" />`,null,true);
    }else{
      const url=await Drive.webViewLink(id);
      if(url!=='#')window.open(url,'_blank');else App.toast('Vista previa disponible al conectar Drive');
    }
  },
  async subir(files){
    if(!files.length)return;
    for(const f of files){
      App.toast(`Subiendo ${f.name}...`);
      try{await Drive.uploadFile(f,this._cur);await Data.agregarActividad('archivo',`<strong>Archivo subido:</strong> ${f.name}`,'📄','#e1f5ee');App.toast(`${f.name} subido`);}
      catch(e){App.toast(`Error al subir: ${f.name}`);}
    }
    await this._cargar(this._cur);
  },
  nuevaCarpeta(){
    App.modal('Nueva carpeta',`<div class="fg"><label class="fl">Nombre de la carpeta</label><input class="fi" id="fn" placeholder="ej. Acme Corp" /></div>`,async()=>{
      const n=document.getElementById('fn').value.trim();
      if(!n){App.toast('Escribe un nombre');return false;}
      await Drive.ensureFolder(n,this._cur);await this._cargar(this._cur);App.toast('Carpeta creada');
    });
  },
  async eliminar(id,name){
    if(!confirm(`¿Eliminar "${name}"?`))return;
    await Drive.deleteFile(id);
    await Data.agregarActividad('archivo',`<strong>Archivo eliminado:</strong> ${name}`,'🗑','#fcebeb');
    await this._cargar(this._cur);App.toast('Archivo eliminado');
  },
  async versiones(id,name){
    const v=await Drive.getVersions(id);
    App.modal(`Historial de versiones — ${name}`,`<div style="font-size:13px;color:var(--text2);margin-bottom:14px">${v.length} versión${v.length!==1?'es':''} guardada${v.length!==1?'s':''} en Drive</div>${v.map((x,i)=>`<div style="display:flex;align-items:center;gap:12px;padding:9px 0;border-bottom:1px solid var(--border)"><span style="font-size:11px;font-family:'DM Mono',monospace;background:var(--surface2);padding:2px 7px;border-radius:4px">v${v.length-i}</span><span style="flex:1;font-size:13px">${U.fechaHora(x.modifiedTime)}</span><span style="font-size:12px;color:var(--text3)">${U.peso(x.size)}</span></div>`).join('')}`,null,true);
  },
  _bc(){
    const el=document.getElementById('bc');
    if(!el)return;
    el.innerHTML=this._crumb.map((b,i)=>i===this._crumb.length-1?`<span>${b.name}</span>`:`<a onclick="Archivos.irA(${i})">${b.name}</a><span style="color:var(--text3)">/</span>`).join('');
  },
...
const App=(()=>{
  ...
  async function onSignedIn(user,token){
    ...
    try{
      // Si hay una carpeta compartida configurada, usarla como raíz
      // para que todos (Santiago y Elizabeth) suban al mismo lugar.
      // Si no, crear/usar la carpeta personal como antes.
      _root=CONFIG.SHARED_FOLDER_ID||await Drive.ensureFolder(CONFIG.DRIVE_ROOT_FOLDER);
