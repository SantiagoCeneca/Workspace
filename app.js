...
const Auth = (() => {
  ...
  function signIn(){
    // Si ya hay sesión guardada con un token, entrar directo.
    // Si el token estuviera expirado, las llamadas a Drive fallarán y
    // onSignedIn cae al arranque vacío sin romperse.
  ...
  return{init,signIn,signOut,getUser,getToken};
})();
...
const Drive = (() => {
  ...
  async function _r(url,o={}){const res=await fetch(url,{headers:_h(),...o});if(!res.ok)throw new Error('Drive '+res.status);return res.json();}
  ...
  async function webViewLink(id){const r=await _r(`${B}/files/${id}?fields=webViewLink`);return r.webViewLink;}
  ...
  async function deleteFile(id){await fetch(`${B}/files/${id}`,{method:'DELETE',headers:_h()});}
...
const Inicio={render(el){
  ...
  el.innerHTML=`<div class="view active">
    ...
    <div class="stats">
      <div class="sc"><div class="sn">${abiertas.length}</div><div class="sl">Tareas abiertas · ${Perfil.nombre(perfil)}</div></div>
...
const Calendario={
  _a:new Date().getFullYear(),_m:new Date().getMonth(),_eventos:[],_eventosCeneca:[],_filtro:'todas',
  async render(el){
    el.innerHTML=`<div class="view active">
  ...
  _grid(){
    ...
    for(let d=1;d<=days;d++){
      ...
      h+=`<div class="cday${ds===hoy?' today':''}"><div class="dnum">${d}</div>
        ${dt.slice(0,2).map(t=>{const c=this._colorTarea(t);return`<div class="cev" style="background:${c.bg};color:${c.fg}" title="${U.esc(t.titulo)}">${t.titulo}</div>`;}).join('')}
        ${ec.slice(0,1).map(e=>`<div class="cev" style="background:#f3e8ff;color:#6b21a8" title="${U.esc(e.summary||'')}">${e.summary||'Evento'}</div>`).join('')}
        ${(dt.length+ec.length)>3?`<div class="cev cevg">+${dt.length+ec.length-3} más</div>`:''}
      </div>`;
    }
  ...
  _renderListas(){
    ...
    if(elC){
      const tareas=this._filtrarTareas(Data.getTareas().filter(t=>!t.done&&t.fecha)).sort((a,b)=>a.fecha.localeCompare(b.fecha)).slice(0,5);
      ...
      if(!tareas.length&&!evC.length){elC.innerHTML='<div style="color:var(--text3);font-size:13px">Sin eventos</div>';}
        ...
        elC.innerHTML=[
          ...tareas.map(t=>{const c=this._colorTarea(t);return`<div style="display:flex;gap:10px;padding:8px 0;border-bottom:1px solid var(--border);align-items:center"><div style="width:8px;height:8px;border-radius:50%;background:${c.fg};flex-shrink:0"></div><div><div style="font-size:13px">${t.titulo}</div><div style="font-size:11px;color:var(--text3)">${U.fecha(t.fecha)} · ${({santiago:'Santiago',elizabeth:'Elizabeth',ambos:'Ambos'})[t.asignado]||'Tarea'}</div></div></div>`;}),
